...
class ComponentManager::RegistrationHandler < ComponentManager
  ...
  def use_invites?
    @entity.settings['use_invites'] || invite_only?
  end
  ...
  def normalize_settings(new_settings)
    ...
    flags.each { |f| result[f] = new_settings[f].to_i == 1 }
    numbers = %w[invite_count]
    numbers.each { |f| result[f] = new_settings[f].to_i }
